...
from thonny.assistanceGPT import SubprocessProgramAnalyzer as SubprocessProgramAnalyzerGPT
...
class MyPyGPTAnalyzer(SubprocessProgramAnalyzerGPT):
    def is_enabled(self):
        return get_workbench().get_option("assistanceGPT.use_mypy")
    ...
    def start_analysis(self, main_file_path, imported_file_paths: Iterable[str]) -> None:
        ...
        env = os.environ.copy()
        mypypath = get_workbench().get_option("assistanceGPT.mypypath")
        if mypypath:
            env["MYPYPATH"] = mypypath
...
def load_plugin():
    ...
    add_program_analyzer_GPT(MyPyGPTAnalyzer)
    get_workbench().set_default("assistanceGPT.use_mypy", True)
    get_workbench().set_default("assistanceGPT.mypypath", None)
