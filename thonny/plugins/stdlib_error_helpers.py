...
from thonny import assistance, assistanceGPT
...
from thonny.assistanceGPT import ErrorHelper as ErrorHelperGPT
from thonny.assistanceGPT import HelperNotSupportedError as HelperNotSupportedErrorGPT
from thonny.assistanceGPT import Suggestion as SuggestionGPT
from thonny.assistanceGPT import add_error_helper as add_error_helper_GPT
from thonny.assistanceGPT import name_similarity as name_similarity_GPT
...
class SyntaxErrorHelperGPT(ErrorHelperGPT):
    def __init__(self, error_info):
        import tokenize
    ...
    def _sug_missing_or_misplaced_colon(self):
        ...
        return SuggestionGPT("missing-or-misplaced-colon", title, body, relevance)
    ...
    def _sug_unbalanced_parens(self):
        ...
        return SuggestionGPT("missing-or-misplaced-colon", "Unbalanced brackets", problem[1], 8)
...
class NameErrorHelperGPT(ErrorHelperGPT):
    def __init__(self, error_info):
        super().__init__(error_info)
    ...
    def _sug_bad_spelling(self):
        ...
        if all_names:
            relevance = 0
            for name in all_names:
                sim = name_similarity_GPT(name, self.name)
                if sim > 4:
                    similar_names.add(name)
                relevance = max(sim, relevance)
        else:
    ...
    def _sug_missing_import(self):
        ...
        return SuggestionGPT("missing-import", "Did you forget to import it?", body, relevance)
...
class AttributeErrorHelperGPT(ErrorHelperGPT):
    def __init__(self, error_info):
        super().__init__(error_info)
        ...
        if len(names) != 2:
            # Can happen eg. in PGZero
            # https://github.com/thonny/thonny/issues/1535
            raise HelperNotSupportedErrorGPT()
...
class OSErrorHelperGPT(ErrorHelperGPT):
    def __init__(self, error_info):
        super().__init__(error_info)
...
class TypeErrorHelperGPT(ErrorHelperGPT):
    def __init__(self, error_info):
        super().__init__(error_info)
...
def load_plugin():
    ...
    for name in globals():
        if name.endswith("ErrorHelperGPT") and not name.startswith("_"):
            type_name = name[: -len("HelperGPT")]
            add_error_helper_GPT(type_name, globals()[name])
